...
MODEL_NAME = "lgb_2_lowess_best_model"
MODEL_PATH = os.path.join(BASE_DIR, "AfterSHAP", "BestModel", "LGB", "Lowess", f"{MODEL_NAME}.pkl")
DATA_PATH = os.path.join(BASE_DIR, "Feature/mordred2dfeature_knn.csv")
FEATURE_PATH = os.path.join(BASE_DIR, "SHAP_select", "LGB/feature/optimal_features_lowess_lgb_accuracy_95.csv")
OUTPUT_DIR = os.path.join(BASE_DIR, "AfterSHAP/tsne/leafindex/lgbcomb")
...
if hasattr(model, 'predict'):
    print("Using model.predict(pred_leaf=True) for feature extraction (LightGBM detected)")
    ...
    raise ValueError("Leaf indices cannot be extracted from the model.")

# ───── Run t-SNE (perplexity=95) ─────
perplexity = 95
...
#  Title and Font Size
plt.title("t-SNE of LightGBM", fontsize=20)
...
output_path = os.path.join(OUTPUT_DIR, f"lgbm_leaf_tsne_final_p{perplexity}.png")
plt.tight_layout()
...
# Purity (simple 2-cluster split based on x-axis median)
...
metrics_df.to_csv(os.path.join(OUTPUT_DIR, f"lgb_tsne_evaluation_p{perplexity}.csv"), index=False)
