...
# ── File paths ──────────────────────────────────────────────
input_file_path = "C:/Users/LG_LAB/Desktop/SSELCPP/Feature/mordred2d_normalized.csv"
importance_file_path = "C:/Users/LG_LAB/Desktop/SSELCPP/Filtermethod/Featureimportance/feature_importance_3.csv"
...
#  Function: compare Kendall's tau (NaN-safe)
def compare_kendall(pair):
    f1, f2 = pair
    x1, x2 = df[f1], df[f2]
    y = df['label']
    ...
    # Keep the feature with the higher tau value
    return f1 if (pd.isna(tau1) or tau1 < tau2) else f2

#  Parallel execution
if __name__ == '__main__':
    with Pool(processes=min(5, cpu_count())) as pool:
        features_to_remove = pool.map(compare_kendall, high_corr_pairs)
    ...
    #  Sort by Kendall’s tau (descending, keeping sign)
