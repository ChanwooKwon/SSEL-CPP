...
# ── Settings ─────────────────────────────────────────────
SHAP_DIR = current_dir / "test_results" / "shap_analysis_mlcpp"
SHAP_DIR.mkdir(parents=True, exist_ok=True)

FEATURE_DIR = base_dir / "shap_feature_selection" / "feature_optimization"
TEST_DATA_PATH = base_dir / "data" / "mlcpp2_test.csv"
...
for model_name in model_names:
    shap_path = SHAP_DIR / f"shap_values_mlcpp_{model_name}.csv"
    ...
    try:
        ...
        save_prefix = SHAP_DIR / f"mlcpp_shap_{model_name}"
        save_shap_summary_plots(shap_values, X_selected, feature_list, save_prefix)
        print(f"✅ MLCPP SHAP plots saved successfully: {model_name}")
        
    except Exception as e:
        print(f"❌ Error processing {model_name}: {str(e)}")
        continue

print("🎉 MLCPP SHAP visualization completed for all models")
