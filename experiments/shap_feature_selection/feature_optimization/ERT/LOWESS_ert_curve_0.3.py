...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    smooth, opt_feat = get_lowess_optimal(x_vals, y_vals, frac=0.3)
...
plt.tight_layout()
plot_path = SAVE_DIR / "ert_shap_lowess0.3_selection.png"
plt.savefig(plot_path, dpi=1000, bbox_inches='tight')
plt.close()
print(f"✅ LOWESS 0.3 plot saved: {plot_path}")
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = SAVE_DIR / f"optimal_features_lowess0.3_ert_{metric.lower()}_{opt_k}.csv"
    pd.DataFrame(feature_list, columns=["Feature"]).to_csv(save_path, index=False)

# 🔹 Summary
print("\n📊 LOWESS 0.3 Optimization Results:")
...
lowess_summary = pd.DataFrame({
    ...
    'LOWESS_0.3_Optimal': [lowess_results['Accuracy'], lowess_results['ROC_AUC'], lowess_results['MCC']]
})
lowess_summary.to_csv(SAVE_DIR / "ert_lowess0.3_summary.csv", index=False)
print(f"✅ Summary saved: {SAVE_DIR / 'ert_lowess0.3_summary.csv'}")