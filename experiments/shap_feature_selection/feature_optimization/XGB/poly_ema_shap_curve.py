...
RANKED_FEATURES_PATH = base_dir / "shap_feature_selection" / "results" / "XGB" / "full_train" / "ranked_features.csv"
SCORE_CSV_PATH = SAVE_DIR / "xgb_performance_by_features.csv"
PLOT_PATH = SAVE_DIR / "xgb_feature_curve_plot.png"
...
# ── Curve Fitting Function to Find Maxima ──────────────
...
# ── Optimal Feature Count via Polynomial ──────────────
...
print(f"Visualization of optimal feature counts saved: {PLOT_PATH}")

# ── Save Optimal Feature Lists ─────────────────────
# EMA
pd.DataFrame(ranked_features[:acc_ema_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_ema_xgb_acc_{acc_ema_opt}.csv", index=False)
pd.DataFrame(ranked_features[:roc_ema_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_ema_xgb_roc_{roc_ema_opt}.csv", index=False)
pd.DataFrame(ranked_features[:mcc_ema_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_ema_xgb_mcc_{mcc_ema_opt}.csv", index=False)

# PolyFit
pd.DataFrame(ranked_features[:acc_poly_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_polyfit_xgb_acc_{acc_poly_opt}.csv", index=False)
pd.DataFrame(ranked_features[:roc_poly_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_polyfit_xgb_roc_{roc_poly_opt}.csv", index=False)
pd.DataFrame(ranked_features[:mcc_poly_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_polyfit_xgb_mcc_{mcc_poly_opt}.csv", index=False)
