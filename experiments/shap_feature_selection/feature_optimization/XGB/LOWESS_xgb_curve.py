...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    # Axis labels and title font sizes
    plt.xlabel("Number of Features", fontsize=20)
    plt.ylabel(metric, fontsize=20)
    plt.title(f"XGBoost LOWESS0.5", fontsize=22)

    # Tick font sizes
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend in bottom right, font size adjustment
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = SAVE_DIR / "xgb_shap_lowess0.5_selection.png"
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = SAVE_DIR / f"optimal_features_lowess_xgb_{metric.lower()}_{opt_k}.csv"
