...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"RandomForest LOWESS0.2", fontsize=22)

    # Tick label font size
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend bottom-right with font size
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = SAVE_DIR / "rf_shap_lowess0.2_selection.png"
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = SAVE_DIR / f"optimal_features_lowess0.2_rf_{metric.lower()}_{opt_k}.csv"
