...
PERF_PATH = SAVE_DIR / "rf_performance_by_features.csv"
...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"RandomForest EMA", fontsize=22)
...
plt.tight_layout()
ema_plot_path = SAVE_DIR / "rf_shap_ema_selection.png"
...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.xlabel("Number of Features", fontsize=20)
    plt.ylabel(metric, fontsize=20)
    plt.title(f"RandomForest Polynomial", fontsize=22)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    plt.grid(True)
    plt.legend(fontsize=18, loc="lower right")
...
plt.tight_layout()
poly_plot_path = SAVE_DIR / "rf_shap_polyfit_selection.png"
