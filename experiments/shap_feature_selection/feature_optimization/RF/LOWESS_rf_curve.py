...
# 🔹 Path Settings
PERF_PATH = current_dir / "curve" / "rf_performance_by_features.csv"
FEATURE_PATH = base_dir / "shap_feature_selection" / "results" / "RF" / "full_train" / "ranked_features.csv"
...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"RandomForest LOWESS0.5", fontsize=22)

    # Tick font sizes
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend in the bottom-right with adjusted font size
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = SAVE_DIR / "rf_shap_lowess_selection.png"
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = SAVE_DIR / f"optimal_features_lowess_rf_{metric.lower()}_{opt_k}.csv"
