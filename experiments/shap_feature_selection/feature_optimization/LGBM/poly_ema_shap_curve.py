...
RANKED_FEATURES_PATH = base_dir / "shap_feature_selection" / "results" / "LGBM" / "full_train" / "ranked_features.csv"
SCORE_CSV_PATH = SAVE_DIR / "lgb_performance_by_features.csv"
PLOT_PATH = SAVE_DIR / "lgb_feature_curve_plot.png"
...
# ── Optimal Feature Count (Polynomial Fit) ──────────────
...
pd.DataFrame(ranked_features[:acc_ema_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_ema_lgb_acc_{acc_ema_opt}.csv", index=False)
pd.DataFrame(ranked_features[:roc_ema_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_ema_lgb_roc_{roc_ema_opt}.csv", index=False)
pd.DataFrame(ranked_features[:mcc_ema_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_ema_lgb_mcc_{mcc_ema_opt}.csv", index=False)

# PolyFit
pd.DataFrame(ranked_features[:acc_poly_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_polyfit_lgb_acc_{acc_poly_opt}.csv", index=False)
pd.DataFrame(ranked_features[:roc_poly_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_polyfit_lgb_roc_{roc_poly_opt}.csv", index=False)
pd.DataFrame(ranked_features[:mcc_poly_opt], columns=["Feature"]).to_csv(
    SAVE_DIR / f"optimal_features_polyfit_lgb_mcc_{mcc_poly_opt}.csv", index=False)
