...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"LightGBM LOWESS0.5", fontsize=22)

    # Tick label font sizes
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend bottom-right position, font size adjustment
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = SAVE_DIR / "lgb_shap_lowess0.5_selection.png"
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = SAVE_DIR / f"optimal_features_lowess_lgb_{metric.lower()}_{opt_k}.csv"
