...
# ── Path settings ─────────────────────────────────────────────
FEATURE_CSV_PATH = base_dir / "filtermethod" / "filtermethod4" / "feature_importance_4.csv"
DATA_PATH = base_dir / "data" / "mordred2dfeature_knn.csv"
SAVE_DIR = current_dir / "results"
SAVE_DIR.mkdir(parents=True, exist_ok=True)

MODEL_SAVE_PATH = SAVE_DIR / "ert_method4_model.pkl"
PARAMS_TXT_PATH = SAVE_DIR / "ert_method4_params.txt"
PROBA_SAVE_PATH = SAVE_DIR / "ert_method4_oof_proba.npy"
LABEL_SAVE_PATH = SAVE_DIR / "ert_method4_labels.npy"
METRIC_TXT_PATH = SAVE_DIR / "ert_method4_metrics.txt"
PLOT_SAVE_PATH = SAVE_DIR / "ert_method4_optimization_history.png"

# ── Load dataset with FilterMethod4 features ──────────────────
...
print(f"FilterMethod4 Dataset: {X.shape}")
...
with open(PARAMS_TXT_PATH, "w") as f:
    f.write("ExtraTrees FilterMethod4 Best Hyperparameters (Optuna)\n")
    f.write("=" * 50 + "\n")
...
# ── Train final model ───────────────────────────────────────
best_params.update({"random_state": 0, "n_jobs": 7})
...
metrics["method"] = "FilterMethod4"
metrics["model"] = "ERT"

with open(METRIC_TXT_PATH, "w") as f:
    f.write("ExtraTrees FilterMethod4 CV Metrics\n")
    f.write("=" * 40 + "\n")
    f.write(f"Features Used: {len(selected_features)} (FilterMethod4)\n\n")
    for key, val in metrics.items():
        if key not in ["method", "model"]:
            f.write(f"{key}: {val:.4f}\n")

print(f"✅ ExtraTrees FilterMethod4 training completed!")
