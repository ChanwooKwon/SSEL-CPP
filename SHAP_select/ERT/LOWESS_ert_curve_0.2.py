...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    # Legend in lower right corner, adjusted font size
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = os.path.join(SAVE_DIR, "ert_shap_lowess0.2_selection.png")
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = os.path.join(SAVE_DIR, f"optimal_features_lowess0.2_ert_{metric.lower()}_{opt_k}.csv")
