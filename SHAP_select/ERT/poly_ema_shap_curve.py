...
# ── Settings ────────────────────────────────────────
BASE_DIR = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_select"
DATA_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/Feature/mordred2dfeature_knn.csv"
RANKED_FEATURES_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_Results/ERT/full_train/ranked_features.csv"
SCORE_CSV_PATH = os.path.join(BASE_DIR, "ert_performance_by_features.csv")
PLOT_PATH = os.path.join(BASE_DIR, "ert_feature_curve_plot.png")
...
# ── Function to find maxima with polynomial curve fitting ──────────────
...
# ── EMA-based optimal number of features ────────────────────
ema_span = 10
...
# ── Polynomial-based optimal number of features ──────────────
...
pd.DataFrame(ranked_features[:acc_ema_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_ema_ert3_acc_{acc_ema_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:roc_ema_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_ema_ert3_roc_{roc_ema_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:mcc_ema_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_ema_ert3_mcc_{mcc_ema_opt}.csv"), index=False)

# PolyFit
pd.DataFrame(ranked_features[:acc_poly_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_polyfit_ert3_acc_{acc_poly_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:roc_poly_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_polyfit_ert3_roc_{roc_poly_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:mcc_poly_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_polyfit_ert3_mcc_{mcc_poly_opt}.csv"), index=False)
