...
# ───── Settings ─────
BASE_DIR = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_select/CB"
PERF_PATH = os.path.join(BASE_DIR, "curve/cb_performance_by_features.csv")
...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"CatBoost EMA", fontsize=22)
...
plt.tight_layout()
ema_plot_path = os.path.join(BASE_DIR, "cb_shap_ema_selection.png")
plt.savefig(ema_plot_path, dpi=1000)
plt.show()
print(f" EMA plot saved: {ema_plot_path}")

# ───── Save Polynomial-based Plots ─────
plt.figure(figsize=(18, 5))
...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.xlabel("Number of Features", fontsize=20)
    plt.ylabel(metric, fontsize=20)
    plt.title(f"CatBoost Polynomial", fontsize=22)
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)
    plt.grid(True)
    plt.legend(fontsize=18, loc="lower right")
...
plt.tight_layout()
poly_plot_path = os.path.join(BASE_DIR, "cb_shap_polyfit_selection.png")
