...
RANKED_FEATURES_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_Results/CB/full_train/ranked_features.csv"
SCORE_CSV_PATH = os.path.join(BASE_DIR, "cb_performance_by_features.csv")
PLOT_PATH = os.path.join(BASE_DIR, "cb_feature_curve_plot.png")
...
for k in tqdm(feature_counts, desc="🔄 Feature Count"):
    ...
    for train_idx, val_idx in kf.split(X_sub, y):
        model = CatBoostClassifier(
            iterations=746,
            learning_rate=0.2796713743684575,
            depth=7,
            l2_leaf_reg=5.813530551409669,
            border_count=105,
            bagging_temperature=0.18782237345663547,
            random_strength=18.694993185286474,
            verbose=0,
            random_seed=0
        )
...
print(f" Visualization of optimal feature count saved: {PLOT_PATH}")

# Save optimal feature lists
for metric, val in zip([
    "acc_ema", "roc_ema", "mcc_ema",
    "acc_polyfit", "roc_polyfit", "mcc_polyfit"
], [
    acc_ema_opt, roc_ema_opt, mcc_ema_opt,
    acc_poly_opt, roc_poly_opt, mcc_poly_opt
]):
    path = os.path.join(BASE_DIR, f"optimal_features_{metric}_cb_{val}.csv")
    pd.DataFrame(ranked_features[:val], columns=["Feature"]).to_csv(path, index=False)
