...
# 🔹 Path Settings
PERF_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_select/GB/curve/gb_performance_by_features.csv"
FEATURE_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_Results/GB/full_train/ranked_features.csv"
SAVE_DIR = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_select/GB/curve"
...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"GradientBoost LOWESS0.5", fontsize=22)

    # Tick font size
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend position (bottom right) and font size
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = os.path.join(SAVE_DIR, "gb_shap_lowess_selection.png")
plt.savefig(plot_path, dpi=1000)
plt.show()
print(f"LOWESS performance graph saved: {plot_path}")

# 🔹 Save the optimal feature list
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    opt_k = results[metric]
    feature_list = ranked_features[:opt_k]
    save_path = os.path.join(SAVE_DIR, f"optimal_features_lowess_gb_{metric.lower()}_{opt_k}.csv")
