...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    plt.title(f"GradientBoost LOWESS0.3", fontsize=22)

    # Tick font size
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend bottom-right, font size adjustment
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = os.path.join(SAVE_DIR, "gb_shap_lowess0.3_selection.png")
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = os.path.join(SAVE_DIR, f"optimal_features_lowess0.3_gb_{metric.lower()}_{opt_k}.csv")
