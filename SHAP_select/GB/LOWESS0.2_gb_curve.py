...
for i, metric in enumerate(["Accuracy", "ROC_AUC", "MCC"]):
    ...
    # Axis and title font size
    plt.xlabel("Number of Features", fontsize=20)
    plt.ylabel(metric, fontsize=20)
    plt.title(f"GradientBoost LOWESS0.2", fontsize=22)

    # Tick label font size
    plt.xticks(fontsize=18)
    plt.yticks(fontsize=18)

    # Legend at bottom-right, adjust font size
    plt.legend(loc="lower right", fontsize=18)

    plt.grid(True)

plt.tight_layout()
plot_path = os.path.join(SAVE_DIR, "gb_shap_lowess0.2_selection.png")
...
for metric in ["Accuracy", "ROC_AUC", "MCC"]:
    ...
    save_path = os.path.join(SAVE_DIR, f"optimal_features_lowess0.2_gb_{metric.lower()}_{opt_k}.csv")
