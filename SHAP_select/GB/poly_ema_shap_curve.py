...
RANKED_FEATURES_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_Results/GB/full_train/ranked_features.csv"
SCORE_CSV_PATH = os.path.join(BASE_DIR, "gb_performance_by_features.csv")
PLOT_PATH = os.path.join(BASE_DIR, "gb_feature_curve_plot.png")
...
pd.DataFrame(ranked_features[:acc_ema_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_ema_gb_acc_{acc_ema_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:roc_ema_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_ema_gb_roc_{roc_ema_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:mcc_ema_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_ema_gb_mcc_{mcc_ema_opt}.csv"), index=False)

# PolyFit
pd.DataFrame(ranked_features[:acc_poly_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_polyfit_gb_acc_{acc_poly_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:roc_poly_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_polyfit_gb_roc_{roc_poly_opt}.csv"), index=False)
pd.DataFrame(ranked_features[:mcc_poly_opt], columns=["Feature"]).to_csv(
    os.path.join(BASE_DIR, f"optimal_features_polyfit_gb_mcc_{mcc_poly_opt}.csv"), index=False)
