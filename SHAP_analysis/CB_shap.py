...
# ─── Configuration ─────────────────────────────────────────────
MODEL_NAME = "CB_1_optuna_best_model"
MODEL_DIR = "C:/Users/LG_LAB/Desktop/SSELCPP/Method4/BestModel/CB"
PARAMS_TXT = "C:/Users/LG_LAB/Desktop/SSELCPP/Method4/CB/CB_1_optuna_best_params.txt"
DATA_PATH = "C:/Users/LG_LAB/Desktop/SSELCPP/Feature/mordred2dfeature_knn.csv"
SAVE_DIR = f"C:/Users/LG_LAB/Desktop/SSELCPP/SHAP_Results/CB"
    ...
    with open(path, "r") as f:
        for line in f:
            if ":" in line:
                ...
                key = key.strip()
                val = val.strip()
                if key in ["learning_rate", "bagging_temperature", "l2_leaf_reg", "random_strength"]:
                    params[key] = float(val)
                elif key in ["iterations", "depth", "border_count"]:
                    params[key] = int(val)
    params["loss_function"] = "Logloss"
    params["verbose"] = 0
    return params

best_params = load_catboost_params(PARAMS_TXT)
...
for fold, (train_idx, val_idx) in enumerate(tqdm(cv.split(X, y), total=5)):
    ...
    model = CatBoostClassifier(**best_params, random_seed=0)
...
# 2) Compute Mean SHAP across folds
...
# 3) Compute SHAP on full training set
print("[SHAP Full Train]")
model_all = CatBoostClassifier()
model_all.load_model(os.path.join(MODEL_DIR, f"{MODEL_NAME}.cbm"))
...
print("\n SHAP computation and saving completed")
